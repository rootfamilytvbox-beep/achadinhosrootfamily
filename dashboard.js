...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (currentUser.role === 'admin') {
        ...
        if (pending.length === 0) {
          ...
          pendingListEl.querySelectorAll('[data-approve-id]').forEach(btn => {
            btn.addEventListener('click', () => {
              ...
              window.MercadoLivreAuth.approveUser(uid);
          ...
          pendingListEl.querySelectorAll('[data-reject-id]').forEach(btn => {
            btn.addEventListener('click', () => {
              ...
              window.MercadoLivreAuth.rejectUser(uid);
        ...
        approvedListEl.querySelectorAll('[data-delete-id]').forEach(btn => {
          btn.addEventListener('click', () => {
            ...
            if (confirm('Deseja realmente revogar o acesso deste usuário?')) {
              window.MercadoLivreAuth.deleteUser(uid);
    ...
    const pendingInitial = window.MercadoLivreAuth.getPendingUsers();
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (dateFilterBtn) {
    dateFilterBtn.addEventListener('click', () => {
      const currentText = document.getElementById('dateRangeText').innerText;
      const newPeriod = prompt('Selecione o perodo desejado (Ex: 01/06/2025 - 07/06/2025):', currentText);
      if (newPeriod && newPeriod.trim() !== '') {
        document.getElementById('dateRangeText').innerText = newPeriod;
        if (typeof showAlert === 'function') {
          showAlert('Perodo atualizado para: ' + newPeriod, 'success');
        } else {
          alert('Perodo atualizado para: ' + newPeriod);
        }
      }
    });
  }
